...
const App = () => {
  ...
  const obtenerPaquetes = async () => {
    const response = await axios.get('http://web:5000/paquetes');
    setPaquetes(response.data);
  };

  const registrarPaquete = async () => {
    await axios.post('http://web:5000/paquete', { id: idPaquete });
    obtenerPaquetes();
  };

  const actualizarEstadoPaquete = async (id) => {
    await axios.put(`http://web:5000/paquete/${id}`, { estado });
    obtenerPaquetes();
  };
